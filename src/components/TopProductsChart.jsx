import { useEffect, useState, useRef } from 'react';
import { Bar } from 'react-chartjs-2';
import { Chart as ChartJS, BarElement, CategoryScale, LinearScale, Tooltip, Legend } from 'chart.js';
import { getTopProducts } from '../api.js';
import { getBaseChartOptions, getPalette } from '../chartTheme.js';
import useThemeVersion from '../useThemeVersion.js';
import ChartActions from './ChartActions.jsx';

ChartJS.register(BarElement, CategoryScale, LinearScale, Tooltip, Legend);

export default function TopProductsChart() {
  const [products, setProducts] = useState([]);
  const themeV = useThemeVersion();
  const chartRef = useRef(null);

  useEffect(() => {
    getTopProducts().then((res) => setProducts(res.data || []));
  }, []);

  const colors = getPalette(products.length);
  const chartData = {
    labels: products.map((p) => p.title),
    datasets: [
      {
        label: 'Cantidad vendida',
        data: products.map((p) => p.cantidadVendida),
        backgroundColor: colors,
        borderRadius: 6,
      },
    ],
  };

  return (
    <div className="card">
      <div className="flex items-center justify-between mb-3">
        <h2 className="card-title mb-0">Productos más vendidos</h2>
        <ChartActions chartRef={chartRef} data={chartData} fileName="top-products" />
      </div>
      <div className="h-80">
        <Bar ref={chartRef} key={themeV} data={chartData} options={{ ...getBaseChartOptions(), indexAxis: 'y' }} />
      </div>
    </div>
  );
}
