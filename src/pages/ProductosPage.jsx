  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        ...
        <ChartCard title="Productos más vistos" description="Top productos según vistas diarias (soporta filtros de período)">
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
              <thead className="bg-brand-soft/80 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:bg-slate-900 dark:text-slate-400">
                <tr>
                ...
                {topViews.map((item) => (
                  <tr key={item.productCode}>
                    <td className="px-6 py-4 font-semibold flex items-center gap-3">
                      <img src={item.product?.mediaSrc?.[0]} alt={item.product?.title} className="h-10 w-10 rounded-md object-cover" />
                      <span>{item.product?.title || item.productCode}</span>
                    </td>
                    <td className="px-6 py-4">{item.views}</td>
                    <td className="px-6 py-4">
                      <a href={`/productos/${item.product?.productCode || item.productCode}`} className="inline-flex items-center gap-1 text-brand-accent hover:text-brand-primary">Ver ficha <ExternalLink className="h-3 w-3" /></a>
                    </td>
                  </tr>
                ))}
                {topViews.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="px-6 py-10 text-center text-sm text-slate-500">No hay datos de vistas.</td>
                  </tr>
                ) : null}
              </tbody>
            </table>
        ...
        <ChartCard title="Productos menos vistos" description="Productos con menos vistas en el período seleccionado">
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
              <thead className="bg-brand-soft/80 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:bg-slate-900 dark:text-slate-400">
                <tr>
